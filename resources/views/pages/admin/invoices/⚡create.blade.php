...
new #[Layout('layouts.admin-app')] #[Title('Create Invoice')] class extends Component {
    ...
    {
        return 'INV-'.now()->format('Ymd').'-'.$this->nextInvoiceNumber();
    }

    public function previewInvoiceNo(): string
    {
    ...
    public function save(): void
    {
        ...
        $subjectPrefix = trim($this->invoiceTemplate->subject_prefix . ' - ');

        if ($subject === '' || $subject === $subjectPrefix) {
            $subject = $this->invoiceTemplate->subject_prefix . ' - ' . substr($invoiceNo, strlen('INV-'));
        }

        DB::transaction(function () use ($validated, $invoiceNo, $subject) {
    ...
    {
        $this->reset(['user_id', 'company_id', 'customer_search', 'customer_name', 'customer_email', 'customer_phone', 'customer_country', 'company_name', 'subject', 'note', 'terms', 'discount_type', 'discount_value', 'issue_date', 'due_date', 'selected_service_id', 'selected_service_plan_id', 'selected_service_option_id', 'selected_billing_cycle', 'custom_title', 'custom_description', 'custom_quantity', 'custom_unit_price', 'items', 'customer_dropdown_open', 'show_customer_modal', 'modal_name', 'modal_email', 'modal_phone', 'modal_country']);
...
<div>
    <div class="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-h1 font-h1 text-on-surface">Create Invoice</h1>
            <p class="mt-1 text-body-md font-body-md text-secondary">
                Build a custom invoice with multiple services, plans, discounts and customer details.
            </p>
        </div>
        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 space-y-6 lg:col-span-4">
                <div class="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="block font-label-md text-on-surface">Invoice Number</label>

                            <input type="text" wire:model.live="subject" disabled
                                class="w-full rounded border border-outline-variant bg-slate-50 px-4 py-2.5 font-mono text-xs"
                                value="{{ $this->previewInvoiceNo() }}" />
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea wire:model.live="note" rows="4" class="w-full rounded border border-outline-variant px-4 py-2.5"
                                placeholder="Write a short invoice message..."></textarea>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea wire:model.live="terms" rows="4" class="w-full rounded border border-outline-variant px-4 py-2.5"
                                placeholder="Example: Payment is due within 7 days."></textarea>
    ...
    @if ($show_customer_modal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
            <div class="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-xl">
                <h3 class="text-lg font-semibold text-on-surface">Create New Customer</h3>

                <div class="mt-5 grid grid-cols-1 gap-4">
                    <div class="space-y-2">
                        <label class="block font-label-md text-on-surface">Customer Name</label>
                        <input type="text" wire:model.live="modal_name"
                            class="w-full rounded border border-outline-variant px-4 py-2.5" />
                    ...
                    <div class="space-y-2">
                        ...
                        <div wire:ignore x-data="phoneInput('modal_phone', 'modal_country')">
                            <input type="tel" x-ref="input" autocomplete="off"
                                class="w-full rounded border border-outline-variant px-4 py-2.5" />
                ...
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <button type="button" x-data x-on:click="$store.phoneInputs.sync('modal_phone'); $wire.createCustomer()"
                        class="rounded-lg bg-primary px-5 py-2 text-label-md font-label-md text-white shadow-sm transition-opacity hover:opacity-90 cursor-pointer">
                        Create Customer
                    </button>
                </div>
            </div>
