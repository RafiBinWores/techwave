...
new #[Layout('layouts.admin-app')] #[Title('Assign Service')] class extends Component {
    ...
    public ?int $service_booking_id = null;
    ...
    public function mount(): void
    {
        ...
        $booking = ServiceBooking::with('service')->find($bookingId);

        if (! $booking) {
            return;
        }

        $this->service_booking_id = $booking->id;

        if ($booking->service_id) {
            $this->service_id = (string) $booking->service_id;
            $this->serviceSearch = $booking->service?->card_title
                ?? $booking->service?->detail_title
                ?? '';
        }

        $this->notes = $booking->message ?: '';

        if ($booking->email) {
            $user = User::query()
                ->whereRaw('LOWER(email) = ?', [strtolower(trim($booking->email))])
                ->first();

            if ($user) {
                $this->user_id = (string) $user->id;
                $this->userSearch = trim($user->name . ' — ' . $user->email);
            } else {
                $this->userSearch = trim($booking->email);
    ...
    public function save(): void
    {
        ...
        if ($this->service_booking_id && UserService::where('service_booking_id', $this->service_booking_id)->exists()) {
            $this->dispatch('toast', message: 'This booking is already assigned to a service.', type: 'warning');
        ...
        UserService::create([
            ...
            'service_booking_id' => $validated['service_booking_id'] ?? null,
        ...
        if ($this->service_booking_id) {
            ServiceBooking::whereKey($this->service_booking_id)->update([
                'status' => 'completed',
            ]);
        }
...
<div>
    <div class="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-h1 font-h1 text-on-surface">Assign Service</h1>
            <p class="mt-1 text-body-md font-body-md text-secondary">
                Assign a confirmed service to a user after quotation or service booking approval.
            </p>
        </div>
            ...
            <div class="col-span-12 space-y-6 lg:col-span-8">
                <div class="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        {{-- User Search --}}
                        <div class="space-y-2 md:col-span-2" wire:key="assign-user-search-wrapper">
                            ...
                            <div x-data class="relative" @click.outside="$wire.hideUserDropdown()">
                                <div class="flex gap-3">
                                    <div class="relative flex-1">
                                        ...
                                        <input type="text" wire:model.live.debounce.500ms="userSearch"
                                            ...
                                            placeholder="Search by user name, email, or phone..." autocomplete="off" />
                                ...
                                @if ($service_booking_id && ! $user_id && $userSearch)
                                    <div class="mt-2 rounded-lg border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                                        No registered user was matched with this booking email.
                                        Please select an existing user manually.
                                    </div>
                                @endif

                                @if ($showUserResults)
                                    <div wire:key="user-search-dropdown"
                                        ...
                                        <div wire:loading.remove wire:target="userSearch">
                                            @forelse ($this->users() as $user)
                                                <button type="button" wire:key="user-option-{{ $user->id }}"
                        ...
                        {{-- Service Search --}}
                        <div class="space-y-2 md:col-span-2" wire:key="assign-service-search-wrapper">
                            ...
                            <div x-data class="relative" @click.outside="$wire.hideServiceDropdown()">
                                <div class="flex gap-3">
                                    <div class="relative flex-1">
                                        ...
                                        <input type="text" wire:model.live.debounce.500ms="serviceSearch"
                                            wire:focus="showServiceDropdown"
                                            class="w-full rounded border border-outline-variant px-10 py-2.5 font-body-md outline-none transition-all focus:ring-2 focus:ring-[#0F52BA] focus:ring-opacity-10"
                                            placeholder="Search by service title or description..." autocomplete="off" />
                                ...
                                @if ($showServiceResults)
                                    <div wire:key="service-search-dropdown"
                                        class="absolute left-0 right-0 z-50 mt-2 max-h-80 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl">
                                        ...
                                        <div wire:loading.remove wire:target="serviceSearch">
                                            @forelse ($this->services() as $service)
                                                <button type="button" wire:key="service-option-{{ $service->id }}"
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                        ...
                        <button type="submit" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="save">Assign Service</span>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm leading-6 text-secondary">
                                    This assignment is linked with booking ID #{{ $service_booking_id }}. After save,
                                    booking status will be marked completed.
                                </p>
                            </div>
                        </div>
                    </div>
