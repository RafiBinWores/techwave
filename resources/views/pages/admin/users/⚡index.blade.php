...
new #[Layout('layouts.admin-app')] #[Title('Manage Users')] class extends Component {
    use WithPagination;
    ...
    {
        $user = User::withCount('departments')->findOrFail($userId);

        if ($user->departments_count > 0) {
            session()->flash('error', 'This user has assigned departments. Please move users before deleting.');
            return;
        }
    ...
    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="relative">
                ...
                <input type="search" wire:model.live.debounce.400ms="search" placeholder="Search department..."
            ...
            <table class="w-full border-collapse text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($this->users() as $user)
                        <tr wire:key="user-{{ $user->id }}" class="transition-colors hover:bg-slate-50/80">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class=" rounded-full px-2.5 py-1 text-[11px] font-label-sm uppercase">
                                    {{ $user->role->name }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <button type="button" wire:click="toggleStatus({{ $user->id }})"
                                    ...
                                    @else
                                        ...
                                        <span class="text-body-md font-body-md text-on-surface">Suspended</span>
                                    @endif
                                </button>
                            </td>
