...
new #[Layout('layouts.admin-app')] #[Title('Edit Live TV Channel')] class extends Component {
    public LiveTvChannel $liveTvChannel;
    public string $name = '';
    public string $url = '';
    ...
    public function mount(LiveTvChannel $liveTvChannel): void
    {
        $this->liveTvChannel = $liveTvChannel;
        $this->name = $liveTvChannel->name ?? '';
        $this->url = $liveTvChannel->url ?? '';
        $this->category = $liveTvChannel->category ?? 'Bangladeshi';
        $this->is_active = $liveTvChannel->is_active ?? true;
        $this->sort_order = $liveTvChannel->sort_order ?? 0;
    }
    ...
    public function update(): void
    {
        ...
        $this->liveTvChannel->update($validated);
        session()->flash('toast', ['type' => 'success', 'message' => 'Channel updated successfully.']);
        $this->redirectRoute('admin.live-tv-channels.index', navigate: true);
    }

    public function discard(): void
    {
        $this->name = $this->liveTvChannel->name;
        $this->url = $this->liveTvChannel->url;
        $this->category = $this->liveTvChannel->category;
        $this->is_active = $this->liveTvChannel->is_active;
        $this->sort_order = $this->liveTvChannel->sort_order;
...
<div>
    <div class="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
            <h1 class="text-h1 font-h1 text-on-surface">Edit Live TV Channel</h1>
            <p class="mt-1 text-body-md font-body-md text-secondary">Update channel details and availability.</p>
        </div>
        <a href="{{ route('admin.live-tv-channels.index') }}" wire:navigate
            ...
            <span class="material-symbols-outlined text-lg">arrow_back</span> Back to Channels
        </a>
    </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div class="space-y-2">
                                <label
                                    ...
                                    Name</label>
                                <input type="text" wire:model.live="name" placeholder="e.g. Somoy TV"
                    ...
                    <div
                        ...
                        <button type="button" wire:click="discard"
                            class="rounded-lg border border-outline-variant px-5 py-2 text-label-md font-label-md text-on-surface transition-colors hover:bg-slate-50 cursor-pointer">Discard
                            Changes</button>
                        <button type="submit" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="update">Update Channel</span>
