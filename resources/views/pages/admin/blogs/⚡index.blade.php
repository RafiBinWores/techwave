...
new #[Layout('layouts.admin-app')] #[Title('Blog Management')] class extends Component {
    use WithPagination;
    ...
    public function toggleStatus(int $blogId): void
    {
        ...
        $this->dispatch('toast', message: 'Blog status updated successfully.', type: 'success');
    }

    public function toggleFeatured(int $blogId): void
    {
        $blog = Blog::findOrFail($blogId);
        ...
        $this->dispatch(
            'toast',
            message: $blog->fresh()->is_featured ? 'Blog marked as featured.' : 'Blog removed from featured.',
            type: 'success'
        );
    }
    ...
    public function delete(int $blogId): void
    {
        ...
        $this->dispatch('toast', message: 'Blog deleted successfully.', type: 'success');
    }
};
?>
        ...
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
                ...
                <p class="text-xs font-body-md text-secondary md:text-body-md">
                    Create and manage articles, SEO content, announcements, and featured blogs.
                </p>
            </div>

            <div class="flex w-full flex-col gap-4 lg:w-auto lg:flex-row lg:items-center lg:justify-between">
                ...
                <table class="w-full border-collapse text-left">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50/50">
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Blog</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Category</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Author</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Featured</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Status</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm uppercase tracking-wider text-secondary">Published</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($this->blogs() as $blog)
                            <tr wire:key="blog-{{ $blog->id }}" class="transition-colors hover:bg-slate-50/80">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <span class="block max-w-sm truncate text-body-sm font-body-sm text-secondary">
                                ...
                                <td class="px-6 py-4">
                                    @if ($blog->category)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-semibold uppercase text-blue-700">
                                            <span class="material-symbols-outlined text-[15px]">
                                                {{ $blog->category->icon ?: 'category' }}
                                            </span>
                                            {{ $blog->category->name }}
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <button
                                        ...
                                        @class([
                                            ...
                                            'bg-amber-100 text-amber-700 hover:bg-amber-200' => $blog->is_featured,
                                            'bg-slate-100 text-slate-500 hover:bg-slate-200' => ! $blog->is_featured,
                                        ])
                                    >
                                        <span class="material-symbols-outlined text-[13px]">
                                            {{ $blog->is_featured ? 'stars' : 'star' }}
                                        </span>
                                ...
                                <td class="px-6 py-4 font-mono text-body-sm text-secondary">
                                    {{ $blog->published_at?->format('M d, Y') ?? 'Not set' }}
                                </td>
                                        ...
                                        >
                                            ...
                                            >
                                                <span class="material-symbols-outlined text-[18px]">
                                                    {{ $blog->is_featured ? 'star_half' : 'stars' }}
                                                </span>
                                            ...
                                            >
                                                <span class="material-symbols-outlined text-[18px]">
                                                    {{ $blog->is_active ? 'visibility_off' : 'visibility' }}
                                                </span>
                                                {{ $blog->is_active ? 'Make Inactive' : 'Make Active' }}
                                            </button>
                                ...
                                <td colspan="7" class="px-6 py-14 text-center">
                                    <div class="mx-auto flex max-w-sm flex-col items-center">
                                        ...
                                        <p class="mt-1 text-sm text-secondary">
                                            Create your first blog to publish content on your website.
                                        </p>
