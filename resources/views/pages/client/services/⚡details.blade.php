...
new #[Title('Service Details | Techwave')] class extends Component {
    ...
    public function planBuyUrl(ServicePlan $plan, string $billingCycle): ?string
    {
        $candidates = match ($billingCycle) {
        ...
        foreach ($candidates as $candidate) {
            ...
            if ($this->isAutoGeneratedCheckoutUrl($plan, (string) $candidate)) {
                continue;
            }
    ...
    private function isAutoGeneratedCheckoutUrl(ServicePlan $plan, string $candidate): bool
    {
        $defaultCheckoutPath = '/services/'.$this->service->slug.'/checkout/'.$plan->slug;

        $candidatePath = parse_url($candidate, PHP_URL_PATH) ?: '';

        return rtrim((string) $candidatePath, '/') === rtrim($defaultCheckoutPath, '/');
    }
                        ...
                        <div class="{{ $planGridClass }}">
                            @foreach ($displayPlans as $plan)
                                ...
                                <div
                                    ...
                                    @if ($hasMonthlyPrice)
                                        @php
                                            $monthlyBuyUrl = $this->planBuyUrl($plan, 'monthly');
                                            $monthlyHref = $monthlyBuyUrl ?? route('client.services.checkout', [$service->slug, $plan->slug]) . '?billing=monthly';
                                        @endphp
                                        <div x-show="billing === 'monthly'" x-cloak>
                                            <a href="{{ $monthlyHref }}"
                                                @if ($monthlyBuyUrl) target="_blank" rel="noopener" @else wire:navigate @endif
                                    ...
                                    @if ($hasYearlyPrice)
                                        @php
                                            $yearlyBuyUrl = $this->planBuyUrl($plan, 'yearly');
                                            $yearlyHref = $yearlyBuyUrl ?? route('client.services.checkout', [$service->slug, $plan->slug]) . '?billing=yearly';
                                        @endphp
                                        <div x-show="billing === 'yearly'" x-cloak>
                                            <a href="{{ $yearlyHref }}"
                                                @if ($yearlyBuyUrl) target="_blank" rel="noopener" @else wire:navigate @endif
                                                class="mt-6 inline-flex w-full items-center justify-center rounded-full bg-linear-to-r from-blue-500 to-sky-400 px-6 py-3.5 font-semibold text-white shadow-lg shadow-blue-500/30 backdrop-blur-xl transition hover:-translate-y-0.5">
                                                Choose Plan
                                            </a>
                                        </div>
                                    ...
                                    @if (!$hasMonthlyPrice && !$hasYearlyPrice)
                                        @php
                                            $oneTimeBuyUrl = $this->planBuyUrl($plan, 'one_time');
                                            $oneTimeHref = $oneTimeBuyUrl ?? route('client.services.checkout', [$service->slug, $plan->slug]) . '?billing=one_time';
                                        @endphp
                                        <a href="{{ $oneTimeHref }}"
                                            @if ($oneTimeBuyUrl) target="_blank" rel="noopener" @else wire:navigate @endif
                                            class="mt-6 inline-flex w-full items-center justify-center rounded-full bg-linear-to-r from-blue-500 to-sky-400 px-6 py-3.5 font-semibold text-white shadow-lg shadow-blue-500/30 backdrop-blur-xl transition hover:-translate-y-0.5">
                                            Choose Plan
                                        </a>
                                    @endif
