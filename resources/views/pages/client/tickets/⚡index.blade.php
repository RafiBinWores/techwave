...
new #[Layout('layouts.account-app')] class extends Component {
    ...
    {
        return (bool) ($this->activeOrder() ?? $this->activeToolSubscription());
    }

    public function activeToolSubscription(): ?ToolSubscription
    {
    ...
    public function activeSupportTitle(): string
    {
        ...
        $subscription = $this->activeToolSubscription();

        if ($subscription) {
            return $subscription->category?->name . ' Subscription';
        }
    ...
    public function activeSupportSubtitle(): string
    {
        ...
        if ($subscription) {
            $expiry = $subscription->expires_at?->format('M d, Y') ?? 'N/A';

            return 'Your tool subscription is active. Eligible for support until ' . $expiry . '.';
        }

        $latestOrder = $this->latestOrder();

        if (!$latestOrder) {
            return 'You need a paid or active order or an active tool subscription to open a support ticket.';
        }
        ...
        return 'You need an active order within a valid service period or an active tool subscription to open support tickets.';
    }
    ...
    public function createTicket()
    {
        ...
        foreach ($this->images as $image) {
            $path = app(\App\Services\ImageService::class)->optimizeAndStore($image, 'support-tickets/' . $ticket->id, maxWidth: 1920, quality: 85);
...
<div>
    <div class="space-y-6" wire:key="client-ticket-index-{{ $refreshKey }}">
        @php
            ...
            $activeToolSub = $this->activeToolSubscription();
            $latestOrder = $this->latestOrder();
        @endphp
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button type="button" wire:click="openCreateModal" @class([
                ...
                'bg-gradient-to-r from-blue-500 to-sky-400 shadow-blue-500/25 hover:-translate-y-0.5 cursor-pointer' => $this->canOpenTicket(),
        ...
        <div class="client-card p-5">
            <div class="grid gap-3 md:grid-cols-2">
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-blue-100/40">
                        search
                    </span>

                    <input type="search" wire:model.live.debounce.400ms="search" placeholder="Search tickets..."
    ...
    @if ($showCreateModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4 backdrop-blur-sm">
            <div class="w-full max-w-2xl rounded-[28px] border border-white/10 bg-slate-950/95 p-6 shadow-2xl">
                            ...
                            <div class="mt-3 grid grid-cols-3 gap-3">
                                @foreach ($images as $index => $image)
                                    <div wire:key="ticket-preview-image-{{ $index }}"
                                        ...
                                        <img src="{{ $image->temporaryUrl() }}" class="h-24 w-full object-cover">
