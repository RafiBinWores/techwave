...
new #[Layout('layouts.account-app')] class extends Component {
    ...
    public function sendReply(): void
    {
        ...
        foreach ($this->images as $image) {
            $path = app(\App\Services\ImageService::class)->optimizeAndStore($image, 'support-tickets/' . $this->ticket->id, maxWidth: 1920, quality: 85);
...
<div wire:key="client-ticket-chat-{{ $ticket->id }}-{{ $refreshKey }}" x-data="{
    ...
    x-on:ticket-replied.window="scrollToBottom()" x-on:keydown.escape.window="closeImagePreview()">
     <div
        class="mx-auto grid w-full max-w-350 grid-cols-1 gap-4 px-3 py-3 sm:px-4 sm:py-4 lg:px-5 xl:grid-cols-[minmax(0,1fr)_320px]">

        {{-- Chat Main --}}
        <div
            class="flex h-[calc(100dvh-1.5rem)] min-h-140 min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/6 backdrop-blur-xl sm:h-[calc(100dvh-2rem)] sm:rounded-[28px] xl:h-[calc(100vh-150px)]">
            ...
            <div x-ref="chatBox"
                ...
                <div class="flex items-end justify-end gap-2 sm:gap-3">
                    <div class="max-w-[82%] text-right sm:max-w-[74%] lg:max-w-[68%]">
                        ...
                        @if ($ticket->message)
                            <div
                                class="inline-block max-w-full wrap-break-word rounded-2xl rounded-br-sm bg-linear-to-r from-blue-500 to-sky-400 px-4 py-1 md:py-3 text-left text-sm leading-relaxed text-white shadow-lg shadow-blue-500/15">
            ...
            <div class="border-t border-white/10 p-2.5 sm:p-4">
                @if ($ticket->status !== 'closed')
                    @if ($images)
                        <div
                            class="mb-3 flex max-w-full gap-2 overflow-x-auto rounded-2xl border border-white/10 bg-white/5 p-2">
            ...
            <div class="client-card p-4 sm:p-6">
                ...
                <div class="mt-5 space-y-4 text-sm sm:mt-6">
                    ...
                    <div class="flex justify-between gap-4">
                        ...
                        <span
                            class="text-right font-semibold text-white">{{ $ticket->created_at?->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
