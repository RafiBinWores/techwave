...
new #[Title('Invoice Generator')] class extends Component {
    ...
    public function saveProductFromModal(): void
    {
        ...
        $this->reset('product_modal_name', 'product_modal_description', 'product_modal_type', 'product_modal_unit', 'product_modal_price', 'product_modal_discount_price', 'product_modal_stock_count', 'product_modal_purchase_price');
    ...
    public function download()
    {
        ...
        if ($theme->usesCustomTemplate()) {
            $html = app(InvoiceThemeRenderer::class)->render($theme, $validated, $items, $subtotal, $taxTotal, $total, $logoDataUri);
        ...
        return response()->streamDownload(fn() => print $pdf->output(), Str::slug($validated['invoice_number']) . '.pdf', ['Content-Type' => 'application/pdf']);
    }
    ...
    private function logoDataUri(): ?string
    {
        ...
        return \App\Services\UploadStorage::dataUri($this->invoice_logo);
    }
};
?>
...
<div class="min-h-screen text-white">
    <section class="mx-auto max-w-350 px-4 py-10 sm:px-6 lg:px-8">
        <div class="mb-10 text-center">
            <h1 class="mt-3 text-4xl font-extrabold sm:text-6xl">Invoice
        ...
        <section>
            <div class="mb-6 flex items-end justify-between gap-4">
                ...
                <span class="hidden text-sm text-blue-100/50 sm:block">{{ $this->themes->count() }} designs
                    available</span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 sm:gap-6 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <article x-data="{ actionsVisible: false }"
                    ...
                    x-on:click.outside="actionsVisible = false" @class([ 'group relative h-72 overflow-hidden rounded-2xl border bg-slate-100 shadow-[0_24px_70px_rgba(0,0,0,0.22)] transition sm:rounded-3xl lg:h-[450px]' , 'border-white/10 hover:-translate-y-1 hover:border-cyan-300/35'=> !$locked,
                    ...
                    <div class="absolute inset-0 overflow-hidden bg-slate-100">
                        ...
                        <img src="{{ \App\Services\UploadStorage::url($theme->preview_image) }}"
                        ...
                        <div class="flex h-full items-center justify-center">
                            <span
                                class="material-symbols-outlined text-7xl text-slate-300">receipt_long</span>
                        </div>
                        @endif
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <p class="mt-1 hidden line-clamp-2 text-sm leading-6 text-blue-100/70 sm:block">
                                {{ $theme->description ?: 'Professional invoice template' }}
                            </p>
                        </div>
                ...
                @empty
                <div
                    class="col-span-full rounded-3xl border border-dashed border-white/15 bg-white/[0.04] p-12 text-center">
                    ...
                    <p class="mt-2 text-blue-100/50">Please check back after an administrator publishes a theme.
                    </p>
                </div>
                @endforelse
            </div>
            @error('selectedThemeId')
            <p class="mt-4 text-center text-sm text-red-300">{{ $message }}</p>
            @enderror
        </section>
        @else
        @php($chosenTheme = $this->themes->firstWhere('id', $selectedThemeId))
        <div
            class="mb-6 flex flex-col gap-4 rounded-2xl border border-cyan-300/20 bg-cyan-400/[0.07] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-4">
                @if ($chosenTheme?->preview_image)
                <img src="{{ \App\Services\UploadStorage::url($chosenTheme->preview_image) }}" alt=""
                    class="h-16 w-20 rounded-lg object-cover object-top">
                @endif
                <div>
            ...
            <button type="button" wire:click="changeTheme"
                class="rounded-lg border border-white/15 px-4 py-2 text-sm font-bold text-blue-100 transition hover:bg-white/10">
                Change theme
            </button>
        </div>
...
<form wire:submit="download" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
    <div class="space-y-6">
        ...
        <section class="rounded-2xl border border-white/10 bg-white/6 p-4 backdrop-blur-xl sm:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div><label class="text-sm text-blue-100/70">Issue date</label><input
                        wire:model="issue_date" type="date"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('issue_date')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                </div>
                <div><label class="text-sm text-blue-100/70">Due date <span
                            class="text-blue-100/40">(optional)</span></label><input wire:model="due_date"
                        type="date"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('due_date')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
        ...
        <section class="rounded-2xl border border-white/10 bg-white/6 p-4 backdrop-blur-xl sm:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="text-sm text-blue-100/70">Company name</label>
                    <input wire:model="seller_company_name" placeholder="Your company name"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                ...
                <div>
                    <label class="text-sm text-blue-100/70">Phone</label>
                    <input wire:model="seller_phone" placeholder="Phone number"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('seller_phone')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input wire:model="seller_email" type="email" placeholder="email@example.com"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('seller_email')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label class="text-sm text-blue-100/70">Website</label>
                    <input wire:model="seller_website" placeholder="https://example.com"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                ...
                <div class="md:col-span-2">
                    <label class="text-sm text-blue-100/70">Address</label>
                    <textarea wire:model="seller_address" rows="2" placeholder="Street, city, zip code, country"
            ...
            <div class="mb-6 grid gap-4 md:grid-cols-2">
                <div class="rounded-xl border border-white/10 bg-white/3 p-4">
                    ...
                    <div class="space-y-3">
                        <div><input wire:model="customer_name" placeholder="Full name"
                                class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                            @error('customer_name')
                            <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                            @enderror
                        </div>
                        <input wire:model="customer_email" type="email" placeholder="Email"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                        <input wire:model="customer_phone" placeholder="Phone"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                        <textarea wire:model="customer_address" rows="3" placeholder="Address"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3"></textarea>
                    </div>
                </div>
                <div class="rounded-xl border border-white/10 bg-white/3 p-4">
                    <div class="mb-3 flex items-center justify-between">
                        ...
                        <button type="button"
                            x-on:click="$wire.set('customer_shipping_name', $wire.customer_name); $wire.set('customer_shipping_email', $wire.customer_email); $wire.set('customer_shipping_phone', $wire.customer_phone); $wire.set('customer_shipping_address', $wire.customer_address)"
                            class="text-xs font-semibold text-cyan-300 hover:text-cyan-200 transition cursor-pointer">
                            Copy from billing
                        </button>
                    </div>
                    <div class="space-y-3">
                        <input wire:model="customer_shipping_name" placeholder="Full name"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                        <input wire:model="customer_shipping_email" type="email"
                            placeholder="Email"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                        <input wire:model="customer_shipping_phone" placeholder="Phone"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                        <textarea wire:model="customer_shipping_address" rows="3" placeholder="Address"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3"></textarea>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full min-w-140 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-white/10">
                        ...
                        <tr wire:key="invoice-item-{{ $index }}" class="bg-black/10">
                            ...
                            <td class="px-3 py-2">
                                <input wire:model.live="items.{{ $index }}.tax" type="number"
                            ...
                            <td class="px-3 py-2 text-center">
                                <button type="button" wire:click="removeItem({{ $index }})"
                                    @disabled(count($items)===1) wire:confirm="Remove this item?"
        ...
        <section class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <textarea wire:model="notes" rows="4" placeholder="Notes"
                    class="w-full rounded-2xl border border-white/10 bg-white/6 p-5"></textarea>
            </div>
            <div>
                <p class="mb-2 text-sm font-bold text-blue-100/70">Terms and conditions</p>
                <textarea wire:model="terms" rows="4" placeholder="Terms and conditions"
                    class="w-full rounded-2xl border border-white/10 bg-white/6 p-5"></textarea>
            </div>
        </section>
    </div>
    ...
    <aside class="space-y-6">
        <section class="rounded-2xl border border-white/10 bg-white/6 p-5 backdrop-blur-xl">
            ...
            <div
                class="mt-4 flex min-h-28 items-center justify-center rounded-xl border border-white/10 bg-white p-4">
                @if ($logo_upload)
                <img src="{{ $logo_upload->temporaryUrl() }}" alt="Invoice logo preview"
                    class="max-h-20 max-w-full object-contain">
                @elseif ($invoice_logo)
                <img src="{{ \App\Services\UploadStorage::url($invoice_logo) }}" alt="Invoice logo"
                    class="max-h-20 max-w-full object-contain">
                @else
            ...
            <div class="mt-4 space-y-3">
                <label for="invoice-logo-upload"
                    ...
                    <span>
                        ...
                        <span class="mt-1 block text-xs text-blue-100/45">PNG, JPG, or WebP up to 2
                            MB</span>
                    </span>
                </label>
                ...
                <p wire:loading wire:target="logo_upload"
                    class="text-center text-xs font-semibold text-cyan-200">
        ...
        <section class="rounded-2xl border border-white/10 bg-white/6 p-5 backdrop-blur-xl">
            ...
            <div class="space-y-4">
                <div><label class="text-sm text-blue-100/70">Currency</label><input wire:model="currency"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('currency')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative mt-2">
                        <input wire:model="discount_value" type="number" min="0"
                            max="{{ $discount_type === 'percentage' ? 100 : 9999999999 }}"
                            step="0.01"
                            class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 pr-12">
                        <span
                            class="absolute inset-y-0 right-4 flex items-center text-sm font-bold text-blue-100/45">
                ...
                <div x-data="{ enabled: {{ $shipping_charge > 0 ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button"
                            x-on:click="enabled = ! enabled; if (!enabled) $wire.set('shipping_charge', 0)"
                    ...
                    <template x-if="enabled">
                        <input wire:model.live="shipping_charge" type="number" min="0"
        ...
        <button type="submit" wire:loading.attr="disabled"
            class="w-full rounded-xl bg-linear-to-r from-cyan-500 to-blue-500 px-6 py-4 font-black uppercase tracking-wider shadow-xl shadow-cyan-500/20 disabled:opacity-60 cursor-pointer">
...
@if ($this->is_premium)
{{-- Customer modal --}}
<div x-data="{ open: false, copyBilling: false }" x-on:open-customer-modal.window="open = true"
    x-on:close-customer-modal.window="open = false; copyBilling = false"
    x-on:keydown.escape.window="open = false" x-cloak x-show="open"
    class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
    <div x-show="open" x-transition.opacity class="absolute inset-0 bg-black/70" x-on:click="open = false">
    </div>
        ...
        <div class="space-y-5">
            ...
            <div class="rounded-xl border border-white/10 bg-white/3 p-4">
                <div class="flex items-center justify-between">
                    ...
                    <label
                        class="flex cursor-pointer items-center gap-2 text-sm text-blue-100/50 hover:text-cyan-200">
                        <input type="checkbox" x-model="copyBilling"
                ...
                <textarea wire:model="customer_modal_shipping_address" rows="3" placeholder="Street, city, zip code, country"
                    x-bind:disabled="copyBilling" x-bind:class="copyBilling ? 'opacity-40' : ''"
                    class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3"></textarea>
            </div>
...
<div x-data="{ open: false }" x-on:open-product-modal.window="open = true"
    x-on:close-product-modal.window="open = false" x-on:keydown.escape.window="open = false" x-cloak
    x-show="open" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
    <div x-show="open" x-transition.opacity class="absolute inset-0 bg-black/70" x-on:click="open = false">
    </div>
    <div x-show="open" x-transition
        ...
        <div class="space-y-4">
            <div>
                <label class="text-sm font-bold text-blue-100/70">Name <span
                        class="text-red-300">*</span></label>
                <input wire:model="product_modal_name"
                    class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                @error('product_modal_name')
                <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                @enderror
            </div>
            <div>
                <label class="text-sm font-bold text-blue-100/70">Description</label>
                <textarea wire:model="product_modal_description" rows="3"
                    class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3"></textarea>
                @error('product_modal_description')
                <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                @enderror
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-bold text-blue-100/70">Type <span
                            class="text-red-300">*</span></label>
                    <select wire:model="product_modal_type"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                ...
                <div>
                    <label class="text-sm font-bold text-blue-100/70">Unit <span
                            class="text-red-300">*</span></label>
                    <select wire:model="product_modal_unit"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-bold text-blue-100/70">Selling price <span
                            class="text-red-300">*</span></label>
                    <input wire:model="product_modal_price" type="number" min="0" step="0.01"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                    @error('product_modal_price')
                    <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label class="text-sm font-bold text-blue-100/70">Discount price</label>
                    <input wire:model="product_modal_discount_price" type="number" min="0"
                        step="0.01"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input wire:model="product_modal_purchase_price" type="number" min="0"
                        step="0.01"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                </div>
                <div>
                    <label class="text-sm font-bold text-blue-100/70">Stock count</label>
                    <input wire:model="product_modal_stock_count" type="number" min="0"
                        step="1"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3">
                </div>
            </div>
