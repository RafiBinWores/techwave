...
new class extends Component
{
    ...
    public function with(): array
    {
        ...
        $activeBackupCount = collect([
            UserCompressedImage::query()->where('user_id', $userId)->where('expires_at', '>', now())->count(),
            UserBgRemovedImage::query()->where('user_id', $userId)->where('expires_at', '>', now())->count(),
            UserResizedImage::query()->where('user_id', $userId)->where('expires_at', '>', now())->count(),
            CompressedPdf::query()->where('user_id', $userId)->where('is_backup_enabled', true)->where('backup_expires_at', '>', now())->count(),
            SplitPdf::query()->where('user_id', $userId)->where('is_backup_enabled', true)->where('backup_expires_at', '>', now())->count(),
            MergedPdf::query()->where('user_id', $userId)->where('is_backup_enabled', true)->where('backup_expires_at', '>', now())->count(),
        ])->sum();
        ...
        return [
            ...
            'activeBackupCount' => $activeBackupCount,
        ];
    }
};
?>
                ...
                <div class="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
                    ...
                    <div class="mb-6 grid grid-cols-2 gap-4 xl:grid-cols-4">
                        <div class="client-card p-5">
                            <p class="text-xs uppercase tracking-wider text-blue-100/45">
                                Active Orders
                            </p>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
                            <a href="{{ route('client.tools.image-compressor') }}" wire:navigate
                                class="rounded-2xl border border-white/10 bg-white/6 p-4 text-center transition hover:bg-white/10">
                                <span class="material-symbols-outlined text-cyan-300">compress</span>
                                <p class="mt-2 text-sm font-semibold text-white">Compress Image</p>
                            </a>
                            ...
                            <a href="{{ route('client.tools.bg-remover') }}" wire:navigate
                                ...
                                <p class="mt-2 text-sm font-semibold text-white">Remove BG</p>
                            </a>

                            <a href="{{ route('client.tools.image-resizer') }}" wire:navigate
                                class="rounded-2xl border border-white/10 bg-white/6 p-4 text-center transition hover:bg-white/10">
                                <span class="material-symbols-outlined text-emerald-300">aspect_ratio</span>
                                <p class="mt-2 text-sm font-semibold text-white">Resize Image</p>
                            </a>

                            <a href="{{ route('client.tools.pdf-splitter') }}" wire:navigate
                                class="rounded-2xl border border-white/10 bg-white/6 p-4 text-center transition hover:bg-white/10">
                                <span class="material-symbols-outlined text-cyan-300">call_split</span>
                                <p class="mt-2 text-sm font-semibold text-white">Split PDF</p>
                            </a>

                            <a href="{{ route('client.tools.pdf-merger') }}" wire:navigate
                                class="rounded-2xl border border-white/10 bg-white/6 p-4 text-center transition hover:bg-white/10">
                                <span class="material-symbols-outlined text-blue-300">merge</span>
                                <p class="mt-2 text-sm font-semibold text-white">Merge PDF</p>
                            </a>

                            <a href="{{ route('client.tools.pdf-compressor') }}" wire:navigate
                                class="rounded-2xl border border-white/10 bg-white/6 p-4 text-center transition hover:bg-white/10">
                                <span class="material-symbols-outlined text-violet-300">picture_as_pdf</span>
                                <p class="mt-2 text-sm font-semibold text-white">Compress PDF</p>
                            </a>
                        </div>
                    </div>
                    ...
                    @if ($sentProposalCount > 0 || $openTickets > 0)
                        <div class="mb-6 rounded-2xl border border-amber-300/20 bg-amber-400/10 p-4">
                            ...
                            <div class="space-y-3">
                                @forelse ($recentOrders as $order)
                                    ...
                                    <div wire:key="dashboard-order-{{ $order->id }}"
                                        class="flex items-start justify-between gap-4 rounded-2xl border border-white/10 bg-white/6 p-4">
                                        <div class="min-w-0">
                                            ...
                                            <p class="mt-1 text-xs text-blue-100/45">
                                                {{ $order->order_no }} &middot; Updated {{ $this->timeAgo($order->updated_at) }}
                                            </p>
                                        </div>
                                    ...
                                    <a href="{{ route('client.tickets.index', ['ticket' => $ticket->id]) }}"
                                        ...
                                        <div class="flex items-start justify-between gap-4">
                                            <div class="min-w-0">
                                                ...
                                                <p class="mt-1 text-xs text-blue-100/45">
                                                    {{ $ticket->ticket_no ?? 'Ticket' }} &middot; {{ $this->timeAgo($ticket->updated_at) }}
                                                </p>
                                            </div>
                            ...
                            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                                @foreach ($activeSubscriptions as $sub)
                                    <div wire:key="dashboard-sub-{{ $sub->id }}"
                                        class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/6 p-4">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="mt-0.5 text-xs text-blue-100/45">
                                                ...
                                                @if ($sub->expires_at)
                                                    &middot; Expires {{ $sub->expires_at->format('d M Y') }}
                                                @endif
                                            </p>
                                        </div>
                                    </div>
