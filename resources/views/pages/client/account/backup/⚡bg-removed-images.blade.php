...
new #[Layout('layouts.account-app')] #[Title('My BG Removed Images')] class extends Component {
    use WithPagination;
    ...
    public function downloadSelected(): mixed
    {
        ...
        foreach ($images as $image) {
            if ($image->fileExists()) {
                $zip->addFromString($image->downloadName(), Storage::disk('public')->get($image->result_path));
            }
        }
        ...
        return response()->download($zipPath, 'bg-removed-images.zip')->deleteFileAfterSend(true);
    }
...
<div x-data="{ preview: null }">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-xl font-bold md:text-h1 text-white">BG Removed Images</h2>
            <p class="text-xs md:text-body-md text-blue-100/60">
                Your background removed image backups.
            </p>
        </div>
    </div>
                                        ...
                                        @forelse ($this->images() as $image)
                                            <tr class="transition hover:bg-white/[0.03]">
                                                ...
                                                <td class="whitespace-nowrap px-4 py-3">
                                                    ...
                                                    <p class="text-xs text-blue-100/40">→ {{ number_format($image->result_size / 1024, 1) }} KB</p>
                                                </td>
