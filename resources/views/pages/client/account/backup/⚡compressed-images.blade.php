...
new #[Title('My Compressed Images')] class extends Component {
    ...
    public function updatedSelectAllVisible(bool $value): void
    {
        ...
        $this->selectedIds = $this->images()->getCollection()->pluck('id')->map(fn($id) => (string) $id)->toArray();
    }

    public function updatedSelectedIds(): void
    {
        $visibleIds = $this->images()->getCollection()->pluck('id')->map(fn($id) => (string) $id)->toArray();

        $this->selectAllVisible = count($visibleIds) > 0 && empty(array_diff($visibleIds, $this->selectedIds));
    }
    ...
    public function deleteSelected(): void
    {
        if (empty($this->selectedIds)) {
            $this->dispatch('toast', message: 'Please select at least one image.', type: 'error');
    ...
    public function downloadSelected(): mixed
    {
        ...
        foreach ($images as $image) {
            if ($image->fileExists()) {
                $zip->addFromString($image->downloadName(), Storage::disk('public')->get($image->compressed_path));
        ...
        if ($addedFiles === 0) {
            ...
            $this->dispatch('toast', message: 'Selected files are no longer available.', type: 'error');
        ...
        return response()->download($zipPath, 'compressed-images.zip')->deleteFileAfterSend(true);
    }

    public function images()
    {
        UserCompressedImage::query()
            ->select(['id', 'user_id', 'compressed_path', 'expires_at'])
            ->where('user_id', auth()->id())
        ...
        return UserCompressedImage::query()
            ->select(['id', 'user_id', 'original_name', 'compressed_path', 'original_size', 'compressed_size', 'compressed_ext', 'expires_at'])
            ->where('user_id', auth()->id())
            ->latest()
...
<div x-data="{ sidebarOpen: false, preview: null }" class="relative min-h-screen text-white">
        ...
        <div
            ...
            <div class="flex min-h-[calc(100vh-3rem)]">
                ...
                <div class="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">
                                    My
                                    <span class="bg-linear-to-r from-cyan-300 to-blue-400 bg-clip-text text-transparent">Compressed Images</span>
                                </h1>
                                <p class="mt-2 text-sm text-blue-100/50">
                                    Manage your backed-up compressed images and download them anytime before expiry.
                                </p>
                            </div>
                        </div>
                    ...
                    {{-- Table --}}
                    <div
                        class="overflow-hidden rounded-3xl border border-white/10 bg-white/6 shadow-[0_16px_60px_rgba(0,0,0,0.18)] backdrop-blur-2xl">
                            ...
                            <table class="w-full min-w-245 border-collapse text-left">
                                <thead>
                                    <tr class="border-b border-white/10 bg-white/4">
                                        <th class="w-14 px-5 py-4">
                                            <label
                                                class="relative flex h-5 w-5 cursor-pointer items-center justify-center">
                                        ...
                                        <th
                                            class="px-5 py-4 text-xs font-semibold uppercase tracking-[0.14em] text-blue-100/45">
                                            Image
                                        </th>
                                        ...
                                        <th
                                            class="px-5 py-4 text-right text-xs font-semibold uppercase tracking-[0.14em] text-blue-100/45">
                                            Action
                                        </th>
                                    </tr>
                                </thead>
                                ...
                                <tbody class="divide-y divide-white/10">
                                    @forelse ($this->images() as $image)
                                        <tr wire:key="user-compressed-image-{{ $image->id }}"
                                            class="transition hover:bg-white/4">
                                            <td class="px-5 py-4">
                                                <label
                                                    class="relative flex h-5 w-5 cursor-pointer items-center justify-center">
                                            ...
                                            <td class="px-5 py-4">
                                                @if ($image->fileExists() && $image->previewUrl())
                                                    <button type="button"
                                                        ...
                                                        <span
                                                            class="absolute inset-0 flex items-center justify-center bg-black/35 text-white opacity-0 transition group-hover:opacity-100">
                                            ...
                                            <td class="px-5 py-4">
                                                <div class="min-w-0 max-w-72">
                                                    ...
                                                    <span
                                                        class="mt-1 inline-flex rounded-md bg-white/8 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-cyan-200">
                                            ...
                                            <td class="px-5 py-4">
                                                @if ($image->isExpired())
                                                    <span
                                                        class="inline-flex items-center gap-2 rounded-full bg-red-500/10 px-3 py-1 text-xs font-semibold text-red-300">
                                                ...
                                                @else
                                                    <span
                                                        ...
                                                        <span
                                                            class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400"></span>
                                        ...
                                        <tr>
                                            <td colspan="8" class="px-6 py-16 text-center">
                                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                                    <div
                                                        class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white/8">
                                                        <span
                                                            class="material-symbols-outlined text-3xl text-blue-100/40">
                                                            backup
                                                        </span>
                                                    </div>
                                                    ...
                                                    <p class="mt-2 text-sm text-blue-100/50">
                                                        Compress images as a premium user to get backup access.
                                                    </p>

                                                    <a href="{{ route('client.tools.image-compressor') }}"
                                                        wire:navigate
                        ...
                        <div
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                @if (count($selectedIds) > 0)
                                    <span
                                        class="rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-semibold text-cyan-200">
    ...
    <template x-teleport="body">
        <div x-show="preview" x-transition.opacity.duration.200ms
            ...
            @click="preview = null" @keydown.escape.window="preview = null" style="display:none;">
